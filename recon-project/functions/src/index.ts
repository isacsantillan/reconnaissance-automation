...
// Start the bot with the provided or prompted token
async function startBot(token?: string) {
  ...
  exports.discordBot = onRequest(
    ...
    async (_req, res) => {
      if (!client.readyTimestamp) {
        const token = process.env.DISCORD_TOKEN || await getToken();
        if (!token) {
...
// Start the bot when run directly (alternative entry point)
if (require.main === module) {
  startBot().catch(console.error);
}